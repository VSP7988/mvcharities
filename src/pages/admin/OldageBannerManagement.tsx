...
import { supabase, OldageBanner } from '../../lib/supabase';

const OldageBannerManagement = () => {
  const [banners, setBanners] = useState<OldageBanner[]>([]);
  const [loading, setLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingBanner, setEditingBanner] = useState<OldageBanner | null>(null);
  ...
  const openModal = (banner?: OldageBanner) => {
    if (banner) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error saving oldage banner:', error);
      alert('Error saving banner. Please try again.');
    }
  };

  const toggleBannerStatus = async (banner: OldageBanner) => {
    try {
      const { error } = await supabase
        .from('oldage_banners')
  ...
  const deleteBanner = async (banner: OldageBanner) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">Oldage Home Banner Management</h1>
                <p className="text-sm text-gray-600">Manage oldage home page banners</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <div key={banner.id} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden">
                <div className="relative">
                  <img
                    ...
                    alt={banner.title || 'Oldage banner'}
        ...
        {banners.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mb-6">Get started by adding your first oldage home banner.</p>
            <button
              onClick={() => openModal()}
...
export default OldageBannerManagement;