...
const ReliefManagement = () => {
  const [reliefContent, setReliefContent] = useState<ReliefContent[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingContent, setEditingContent] = useState<ReliefContent | null>(null);
  ...
  const openModal = (content?: ReliefContent) => {
    if (content) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await loadReliefContent();
      closeModal();
    } catch (error) {
      console.error('Error saving relief content:', error);
      alert('Error saving relief content. Please try again.');
    } finally {
      setSaving(false);
    }
  };

  const toggleContentStatus = async (content: ReliefContent) => {
    try {
      const { error } = await supabase
        .from('relief_content')
  ...
  const deleteContent = async (content: ReliefContent) => {
    if (!window.confirm('Are you sure you want to delete this relief content?')) return;
    ...
    try {
      ...
      console.error('Error deleting relief content:', error);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">Relief Management</h1>
                <p className="text-sm text-gray-600">Manage relief programs content</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          <div className="flex justify-center items-center py-12">
            ...
            <span className="ml-2 text-gray-600">Loading relief content...</span>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {reliefContent.map((content) => (
        ...
        {reliefContent.length === 0 && !loading && (
          <div className="text-center py-12">
            <HandHeart className="h-16 w-16 text-gray-400 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No Relief Content Yet</h3>
            <p className="text-gray-600 mb-6">Get started by adding your first relief content.</p>
            <button
              onClick={() => openModal()}
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-gray-900">
                  {editingContent ? 'Edit Relief Content' : 'Add New Relief Content'}
                </h2>
                <button
                  onClick={closeModal}
                    ...
                    <div key={index} className="p-4 border border-gray-200 rounded-lg">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <input
                          ...
                          placeholder="Icon name (e.g., Utensils)"
                          className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                        />
                      </div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full mt-3 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-vertical"
                      />
                    </div>
                  ))}
                </div>
                    ...
                    <div key={index} className="p-4 border border-gray-200 rounded-lg">
                      ...
                      <div className="space-y-3">
                        <input
                          ...
                          placeholder="Number (e.g., 24/7, 100+)"
                        ...
                        <input
                          ...
                          placeholder="Label (e.g., Response Ready)"
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                        />
                      </div>
                    </div>
