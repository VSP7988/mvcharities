...
import { supabase, ChildrenContent } from '../../lib/supabase';

const ChildrenContentManagement = () => {
  const [childrenContent, setChildrenContent] = useState<ChildrenContent[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingContent, setEditingContent] = useState<ChildrenContent | null>(null);
  ...
  const openModal = (content?: ChildrenContent) => {
    if (content) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await loadChildrenContent();
      closeModal();
    } catch (error) {
      console.error('Error saving children content:', error);
      alert('Error saving children content. Please try again.');
    } finally {
      setSaving(false);
    }
  };

  const toggleContentStatus = async (content: ChildrenContent) => {
    try {
      const { error } = await supabase
        .from('children_content')
  ...
  const deleteContent = async (content: ChildrenContent) => {
    if (!window.confirm('Are you sure you want to delete this children content?')) return;
    ...
    try {
      ...
      console.error('Error deleting children content:', error);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900">Children Home Content Management</h1>
                <p className="text-sm text-gray-600">Manage children home page content</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <span>Add Children Content</span>
              </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading ? (
          <div className="flex justify-center items-center py-12">
            ...
            <span className="ml-2 text-gray-600">Loading children content...</span>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {childrenContent.map((content) => (
        ...
        {childrenContent.length === 0 && !loading && (
          <div className="text-center py-12">
            <Users className="h-16 w-16 text-gray-400 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No Children Content Yet</h3>
            <p className="text-gray-600 mb-6">Get started by adding your first children content.</p>
            <button
              onClick={() => openModal()}
            ...
            >
              Add Your First Children Content
            </button>
          </div>
        )}
      </div>
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-gray-900">
                  {editingContent ? 'Edit Children Content' : 'Add New Children Content'}
                </h2>
                <button
                  onClick={closeModal}
                      ...
                      <div key={index} className="p-4 border border-gray-200 rounded-lg">
                        ...
                        <div className="space-y-3">
                          ...
                          <input
                            ...
                            placeholder="Label (e.g., Children Cared For)"
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                          />
                        </div>
                      </div>
                      ...
                      <div key={index} className="p-4 border border-gray-200 rounded-lg">
                        ...
                        <div className="space-y-3">
                          ...
                          <input
                            ...
                            placeholder="Icon name (e.g., Home, GraduationCap)"
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                          />
                          <textarea
...
export default ChildrenContentManagement;