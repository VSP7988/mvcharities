...
import { supabase, ChildrenGallery } from '../lib/supabase';

const ChildrenGallerySection = () => {
  const [selectedImage, setSelectedImage] = useState<any>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const [childrenGalleryImages, setChildrenGalleryImages] = useState<ChildrenGallery[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    loadChildrenGalleryImages();
  }, []);

  const loadChildrenGalleryImages = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase
        .from('children_gallery')
        .select('*')
        .eq('is_active', true)
        .order('sort_order', { ascending: true });

      if (error) {
        console.error('Error loading children gallery images:', error);
        return;
      }
      
      setChildrenGalleryImages(data || []);
    } catch (error) {
      console.error('Error loading children gallery images:', error);
    } finally {
      setLoading(false);
  ...
  const defaultGalleryImages = [
    ...
    },
    {
      ...
      description: "Fun and recreational activities for children"
    },
    {
      id: 3,
      ...
      description: "Loving care and support for all children"
    },
    {
      id: 4,
    ...
    },
    {
      ...
      description: "Ensuring the health and wellbeing of children"
    },
    {
      id: 6,
      src: "https://images.pexels.com/photos/4173244/pexels-photo-4173244.jpeg?auto=compress&cs=tinysrgb&w=800",
      title: "Safe Environment",
      description: "Providing a safe and secure living environment"
    }
  ];

  // Use children gallery images if available, otherwise use default gallery images
  const displayImages = childrenGalleryImages.length > 0 
    ? childrenGalleryImages.map((img, index) => ({
        id: img.id,
        src: img.image_url,
        title: `Children Gallery Image ${index + 1}`,
        description: "Children home activities and care services"
      }))
...
export default ChildrenGallerySection;